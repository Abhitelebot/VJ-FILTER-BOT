        ...
        if not manual:
            ...
            try:
                if settings['auto_ffilter']:
                    ai_search = True
                    reply_msg = await message.reply_text(f"<b><i>Searching For your requested movie/series 🔍</i></b>")
    ...
    if save == True and SAVE_RESTRICTED_MODE == True:
        try:
            ...
            if not link:
                ...
                if PM_SEARCH == True:
                    ai_search = True
                    reply_msg = await bot.send_message(message.from_user.id, f"<b><i>Searching For your requested movie/series🔍</i></b>", reply_to_message_id=message.id)
                    await auto_filter(bot, content, message, reply_msg, ai_search)
                else:
                    await message.reply_text(
                        text=f"<b>ʜᴇʏ {user} 😍 ,\n\nʏᴏᴜ ᴄᴀɴ'ᴛ ɢᴇᴛ ᴍᴏᴠɪᴇs ꜰʀᴏᴍ ʜᴇʀᴇ. ʀᴇǫᴜᴇsᴛ ɪᴛ ɪɴ ᴏᴜʀ ᴍᴏᴠɪᴇ ɢʀᴏᴜᴘ ᴏʀ ᴄʟɪᴄᴋ ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ʙᴜᴛᴛᴏɴ ʙᴇʟᴏᴡ 👇</b>",
                        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📝 ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ", url="https://t.me/A2Zallmovierequest")]])
                    )
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    # don't change anything without contacting me @kingvj01
        ...
        await query.message.edit_text(
            ...
            btn = [[
                    InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="help"),
                    InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="https://t.me/atozmoviesrequest")
                  ]]
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
...
async def auto_filter(client, name, msg, reply_msg, ai_search, spoll=False):
        ...
        if len(message.text) < 100:
            ...
            for x in find:
                if x in removes:
                    ...
                    search = search + x + " "
            search = re.sub(r"\b(pl(i|e)*?(s|z+|ease|se|ese|(e+)s(e)?)|((send|snd|giv(e)?|gib)(\sme)?)|movie(s)?|latest|bruh|broh|helo|that|find|dubbed|link|venum|iruka|pannunga|pannungga|anuppunga|anupunga|anuppungga|anupungga|film|undo|kitti|kitty|tharu|kittumo|kittum|movie|any(one)|with\ssubtitle(s)?)", "", search, flags=re.IGNORECASE)
            search = re.sub(r"\s+", " ", search).strip()
            search = search.replace("-", " ")
            search = search.replace(":", "")
            search = search.replace(".", "")
            search = unicodedata.normalize('NFKD', search).encode('ascii', 'ignore').decode('utf-8')
    ...
    FRESH[key] = search
    temp.GETALL[key] = files
    user_id = message.from_user.id if (message and message.from_user) else (msg.from_user.id if msg.from_user else 0)
    ...
    try:
        ...
        logger.error(f"Error getting poster in auto_filter: {e}")
        imdb = None
    cur_time = datetime.now(pytz.timezone('Asia/Kolkata')).time()
    time_difference = timedelta(hours=cur_time.hour, minutes=cur_time.minute, seconds=(cur_time.second+(cur_time.microsecond/1000000))) - timedelta(hours=curr_time.hour, minutes=curr_time.minute, seconds=(curr_time.second+(curr_time.microsecond/1000000)))
    remaining_seconds = "{:.2f}".format(time_difference.total_seconds())
    ...
    if imdb:
        ...
        mention_str = message.from_user.mention if (message and message.from_user) else (msg.from_user.mention if (msg and msg.from_user) else "User")
        chat_title = message.chat.title if message.chat.title else "PM"
        if settings["button"]:
            cap = f"<b>Tʜᴇ Rᴇꜱᴜʟᴛꜱ Fᴏʀ ☞ {search}\n\nRᴇǫᴜᴇsᴛᴇᴅ Bʏ ☞ {mention_str}\n\nʀᴇsᴜʟᴛ sʜᴏᴡ ɪɴ ☞ {remaining_seconds} sᴇᴄᴏɴᴅs\n\nᴘᴏᴡᴇʀᴇᴅ ʙʏ ☞ : {chat_title} \n\n⚠️ ᴀꜰᴛᴇʀ 5 ᴍɪɴᴜᴛᴇꜱ ᴛʜɪꜱ ᴍᴇꜱꜱᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴀᴜᴛᴏᴍᴀᴛɪᴄᴀʟʟʏ ᴅᴇʟᴇᴛᴇᴅ 🗑️\n\n</b>"
        else:
            cap = f"<b>Tʜᴇ Rᴇꜱᴜʟᴛˢ Fᴏʀ ☞ {search}\n\nRᴇǫᴜᴇsᴛᴇᴅ Bʏ ☞ {mention_str}\n\nʀᴇsᴜʟᴛ sʜᴏᴡ ɪɴ ☞ {remaining_seconds} sᴇᴄᴏɴᴅs\n\nᴘᴏᴡᴇʀᴇᴅ ʙʏ ☞ : {chat_title} \n\n⚠️ ᴀꜰᴛᴇʀ 5 ᴍɪɴᴜᴛᴇꜱ ᴛʜɪꜱ ᴍᴇꜱꜱᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴀᴜᴛᴏᴍᴀᴛɪᴄᴀʟʟʏ ᴅᴇʟᴇᴛᴇᴅ 🗑️\n\n</b>"
    ...
    if imdb and imdb.get('poster'):
        try:
            if spoll:
                hehe = await client.send_photo(chat_id=message.chat.id, photo=imdb.get('poster'), caption=cap, reply_markup=InlineKeyboardMarkup(btn))
            else:
            ...
            if spoll:
                hmm = await client.send_photo(chat_id=message.chat.id, photo=poster, caption=cap, reply_markup=InlineKeyboardMarkup(btn))
            else:
            ...
            logger.exception(e) 
            if spoll:
                fek = await client.send_message(chat_id=message.chat.id, text=cap, reply_markup=InlineKeyboardMarkup(btn))
            else:
